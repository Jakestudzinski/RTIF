...
export async function GET(request: NextRequest) {
  ...
  const allowedDomains = (process.env.GATEWAY_ALLOWED_REDIRECT_DOMAINS || '').split(',').map(d => d.trim()).filter(Boolean)
  if (allowedDomains.length > 0 && !allowedDomains.includes(destUrl.hostname)) {
    console.error(`🔴 [PAYMENT-GATEWAY] Redirect blocked — ${destUrl.hostname} not in allowed domains: ${allowedDomains.join(', ')}`)
  ...
  console.log(`✅ [PAYMENT-GATEWAY] Redirecting to: ${destUrl.hostname}${destUrl.pathname}`)
